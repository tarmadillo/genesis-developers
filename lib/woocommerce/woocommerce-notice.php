...
function woocommerce_theme_notice() {
	...
	if ( get_user_option( __NAMESPACE__ . '\woocommerce_message_dismissed', get_current_user_id() ) ) {
		return;
	}
...
function reset_woocommerce_notice() {
	...
	$args = array(
		'meta_key'   => $wpdb->prefix . __NAMESPACE__ . '\woocommerce_message_dismissed',
		'meta_value' => 1,
	);
	$users = get_users( $args );

	foreach ( $users as $user ) {
		delete_user_option( $user->ID, __NAMESPACE__ . '\woocommerce_message_dismissed' );
	}
